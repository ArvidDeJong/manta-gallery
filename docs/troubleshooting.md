...
This guide helps you resolve common issues with the Manta Gallery Form package.
...
1. **Add database indexes:**
   ```sql
   CREATE INDEX idx_gallerys_email ON manta_gallerys(email);
   CREATE INDEX idx_gallerys_created_at ON manta_gallerys(created_at);
   CREATE INDEX idx_gallerys_company_id ON manta_gallerys(company_id);
   ```
...
2. **Use pagination for large datasets:**
   ```php
   $gallerys = Gallery::paginate(25);
   ```

## Common Error Messages

### "Class 'Darvis\Mantagallery\Models\gallery' not found"
...
### "Route [gallery.index] not defined"
